...
const statusBadge = (s) => {
    if (s === "Delivered")  return Badge.green(s);
    if (s === "In Transit") return Badge.blue(s);
            ...
            <div className="d-flex gap-2 mb-24 flex-wrap">
                {ORDER_STATUSES.map(t => (
                    <button
                        key={t}
                        onClick={() => setFilter(t)}
                        style={{ padding: "7px 18px", borderRadius: 20, border: "1.5px solid", fontWeight: 600, fontSize: 13, cursor: "pointer", borderColor: filter === t ? ORANGE : "#ddd", background: filter === t ? ORANGE : "#fff", color: filter === t ? "#fff" : "#555" }}
                    >{t}</button>
                ))}
            </div>

            {visible.length === 0
                ? <EmptyState emoji="📦" message="No orders found for this filter." />
                : visible.map(order => (
                    <div key={order.id} style={st.orderRow}>
                        <div style={st.orderImg}>{order.emoji}</div>
                        <div style={{ flex: 1 }}>
                            <p style={{ fontWeight: 600, color: "#222", marginBottom: 2, fontSize: 15 }}>{order.item}</p>
                            <p style={{ color: "#888", fontSize: 13, marginBottom: 4 }}>{order.id} &nbsp;·&nbsp; {order.date}</p>
